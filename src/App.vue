  ...
  <div id="app" class="">
    <v-app id="inspire">
      ...
      <span v-else>
          ...
          class="pl-2 pr-2 grey lighten-5"
          absolute
          floating
      ...
      >
        ...
        >
			...
			<v-main>
				<router-view v-if="!$store.state.initializing"></router-view>
        <!--NEW REPORT DIALOG-->
        <v-dialog
            v-model="reporting"
            width="500"
        ...
        >
            ...
            <v-card-title>
              Generate a new report <v-spacer></v-spacer>
              <v-btn v-if="!creating" @click="reporting=false;
							fileName = '';
							reportPeriod = '';
    ...
    >
              <template v-slot:item="{item}">
                ...
                <template>
                  <v-list-item-avatar rounded>
	                  <v-img contain :src="item.logo" :alt="item.name"></v-img>
                  </v-list-item-avatar>
                  <v-list-item-content>
              ...
              <v-btn dark depressed block :color="templateColor ? templateColor : '#0D47A1'"
	            ...
	            <v-sheet rounded="lg" class="mt-3 pt-5" color="grey lighten-4" v-if="!creating">
		            ...
		            <v-col cols="12" class="d-flex align-center">
		              <img src="/img/qb_logo1.png" width="120" alt="qb" class="white ma-2"/>
		              <img src="/img/Wave_logo_RGB-758x275-296650780.png" width="120" alt="wave" class="white ma-2"/>
		              <img src="/img/zoho-books_q5s8.1920-3895511793.jpg" width="120" alt="zoho" class="white ma-2"/>
		            </v-col>
		            <v-col cols="12" class="d-flex align-center">
		              <img src="/img/xero-logo-1122207467.png" width="120" alt="xero" class="white"/>
		              <img src="/img/freshbooks_hn71.1200-3996272495.jpg" width="120" alt="fb" class="white pa-2 ml-2"/>
		              <img src="/img/Sage_Green_Logo-2228348961.jpg" width="120" alt="sage" class="white pa-2 ml-2"/>
		            </v-col>
	            </v-row>
	            </v-sheet>
            ...
            ></v-img>
                ...
                <h5 class="text-center">Generating report, please wait ...</h5>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      showSidebar: true,
      isChatMode: true, // Initialize the chat mode as false
      isReportMode: false, // Initialize the report mode as false
      isLedgerMode: true,
      fileName: null,
      fileToView: null,
      currentFile: null,
  ...
  computed: {
    ...
    allBusinesses() {
      ...
      return [...this.businesses, ...this.invitedBusinesses];
    },
    businessYears() {


      const selectedBusiness = this.allBusinesses.find(bus => bus.id == this.selectedBusiness);
      ...
      const minYear = selectedBusiness.min_year;
      const maxYear = selectedBusiness.max_year;
      const years = [];
  ...
  methods: {
    ...
    getReports() {
      ...
      axios.get("/api/management-reports")
          .then(res => {
            this.reports = res.data.data.reverse();
    ...
    getInvitedBusinesses() {
      axios.get("/api/business/invited")
          .then(res => {
  ...
  mounted() {
    ...
    eventBus.$on('delete-report', () => {
      this.getReports();
    });

    eventBus.$on('newReport', () => {
      this.reporting = true;
    });
...
<style>
.logo-animation {
  -webkit-animation: fade-in 2.5s cubic-bezier(0.39, 0.575, 0.565, 1) 1s infinite both;
  animation: fade-in 2.5s cubic-bezier(0.39, 0.575, 0.565, 1) 1s infinite both;
